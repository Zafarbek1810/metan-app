...
const EditCashbackTable = ({ cashId }) => {
  ...
  function getCashObj() {
    CashbackProvider.getAllCashback().then((res) => {
  ...
  function getCarTypes() {
    CashbackProvider.getCarTypes().then(({ data }) => {
      ...
      <Wrapper>
        ...
        <div className="content">
          ...
          <h5 style={{ color: "rgba(0, 0, 0, 0.7)" }}>
            Nomi <sup style={{ color: "red" }}>*</sup>
          </h5>
          <input
              autoComplete="off"
            className="input"
            value={cashName}
            onChange={handleChangeCashName}
          />
          <hr />
            ...
            <tbody>
              <tr className="edit_row">
                ...
                <td>
                  <input
                    ...
                    type="number"
                    className="input"
                    {...register("initialPointsForStatus")}
                  />
                </td>
                <td>
                  <button type="submit" className="add-btn" form="addRuleForm">
                    Qo'shish
                  </button>
                </td>
              </tr>
              {cash?.rules?.map((rule) => (
                <tr key={rule.name} className="edit_row">
                  <td>{rule.carType.title}</td>
                  <td>{rule.statusName}</td>
                  <td>{rule.cashbackPercentage}%</td>
                  <td>{rule.initialPointsForStatus}</td>
                  <td align="center">
                    <button
                      className="delete-btn"
                      onClick={() => handleDeleteCashRule(rule.id)}
                    >
                      <DeleteSvg />
                    </button>
                  </td>
